...
019000*-----SIGNX,Y DECIDE WHETHER TO INCREMENT OR DECREMENT.
019100      IF POINT2X - POINT1X < 0 MOVE -1 TO SIGNX.
019200      IF POINT2X - POINT1X NOT < 0 MOVE 1 TO SIGNX.
019300      IF POINT2Y - POINT1Y < 0 MOVE -1 TO SIGNY.
019400      IF POINT2Y - POINT1Y NOT < 0 MOVE 1 TO SIGNY.
019500*-----CHECK WHETHER SLOPE IS INFINITE.
019600      IF POINT2X = POINT1X
019700*-----CASE 1 - SLOPE IS INFINITE
019800          PERFORM KEEP-DRAWING-INFINITE-SLOPE.
019900      IF POINT2X NOT = POINT1X
020000*-----CASE 2 - SLOPE IS FINITE
020100          COMPUTE SLOPE = (POINT2Y - POINT1Y) /
020200                          (POINT2X - POINT1X)
020300          PERFORM KEEP-DRAWING.
020400      MOVE '*' TO BF-POINT(POINT2Y, POINT2X).
020500
020600 KEEP-DRAWING.
020700      IF SLOPE > 0 MOVE SLOPE TO ABS-SLO.
020800      IF SLOPE NOT > 0 COMPUTE ABS-SLO = SLOPE * -1.
020900      MOVE '*' TO BF-POINT(ROW-DRAWON, COL-DRAWON).
021000      IF SLOPE NOT > 1 AND SLOPE NOT < -1
021100*-----INTRODUCE STEP-TO-MOVE TO HANDLE FLOATING POINT ERROR.
021200            COMPUTE STEP-TO-MOVE ROUNDED = CNT * ABS-SLO
021300            COMPUTE ROW-DRAWON ROUNDED = POINT1Y +
021400                                         STEP-TO-MOVE * SIGNY
021500            COMPUTE COL-DRAWON = POINT1X + CNT * SIGNX.
021600      IF SLOPE > 1 OR SLOPE < -1
021700            COMPUTE ROW-DRAWON = POINT1Y + CNT * SIGNY
021800            COMPUTE STEP-TO-MOVE ROUNDED = CNT / ABS-SLO
021900            COMPUTE COL-DRAWON ROUNDED = POINT1X + 
022000                                         SIGNX * STEP-TO-MOVE.
022100      ADD 1 TO CNT.
022200      IF ROW-DRAWON NOT = POINT2Y OR COL-DRAWON NOT = POINT2X
022300          GO TO KEEP-DRAWING.
022400
022500 KEEP-DRAWING-INFINITE-SLOPE.
022600      MOVE '*' TO BF-POINT(ROW-DRAWON, COL-DRAWON).
022700      ADD 1 TO ROW-DRAWON.
022800      IF ROW-DRAWON NOT = POINT2Y 
022900          GO TO KEEP-DRAWING-INFINITE-SLOPE.
023000
023100*===============================================================
023200 WRITE-TO-FILE.
023300      WRITE OUTPUT-RECORD FROM BF-ROWS(CUR-ROW).
023400      COMPUTE CUR-ROW = CUR-ROW - 1.
023500      IF CUR-ROW > 0 GO TO WRITE-TO-FILE.
023600 
023700*===============================================================
023800 OPEN-ERR-HANDLING.
023900      DISPLAY 'File Open Error. Program terminated.'.
024000      STOP RUN.
024100
